...
reached their limit (as described in \cref{sec:sv_detection}) and ongoing
...
\explain{read orientation}{Only one DNA strand (e.g. the $5^\prime$ strand) is
    ...
    Thus, also the reverse complement sequence of each read must be mapped---i.e.
    it maps to the \emph{minus} strand (\cref{fig:reads})} within the reference
