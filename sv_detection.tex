...
phenotypes \citep{Swaminathan2012}. \Acp{sv} were also mapped extensively in
...
\figuretextwidth[t]{SV_limitations.pdf}{sv_limitations}{Examples for limitations
    ...
    a sample with an \sv, the lower part reads from that sample mapping to the
    reference assembly. \textbf{A:} A duplication of a repetitive element
    ...
    behind this issue is that only the breakpoint of an \sv (similar scenarios
    are for other \sv classes) are being called, whereas \mps-based methods
    cannot reason about the inner state of the translocation.}
\todo{harmonize the way I refer to subfigures throughout the thesis}
