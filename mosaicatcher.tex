...
clonal expansion of single cells or single-cell genomics techniques. In bulk
...
However, focal \acp{sv} are difficult to assess in this way, as typical
...
laboratory, which are then let grown to obtain population of genotypically identical (clonal) cells.
...
\Cref{fig:ss_library} depicts Strand-seq data from a single cell of an \acl{rpe} (\acp{rpe}-1)
...
includes an example of an inverted duplication within \acp{rpe}-1 wild type cells.
...
between two chromosomes is indicative of a translocation event. In the \acp{rpe}-1 wild
...
\figurearbitrary[t!]{0.5}{mosaic_rpe_mean_var.pdf}{mean_var}{Mean variance
    ...
    \acp{rpe}-1 wild type cell line. Shown here are the mean number of reads per
...
performance. Initial results on \acp{rpe}-1 cells looked very promising, but did not
...
demonstrating its performance in cell line data. Besides the \acp{rpe}-1
...
the community about building atlases of cell types in humans and model organisms,
e.g. the Human Cell Atlas \citep{Regev2017}. Given the relevance of mosaic
\acp{sv} in human disease, \mc may even pave the way for single-cell projects
like the Human Cell Atlas to extend their studies genetic mosaicism and its
