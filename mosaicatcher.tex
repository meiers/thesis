\chapter{Structural Variant Detection in Single Cells}
...
However, mosaicism has also been linked to
...
\subsection{Single-cell Strand-seq libraries}
...
%    \ifodd\value{page} \expandafter\afterpage \fi {%
    \begin{figure}[t!]                                             % cell BM160815_WT_007p1 from BM160815_WT.200000.pdf
        ...
        \figcap{ss_library}{Example of a single cell Strand-seq library}{
            ...
            line is displayed using the plot function of \mc (\cref{sec:mosaic_method}).
            ...
            the right, in blue. In the cell shown here, data reads were binned at
            200~kb-bin contains, leading to a median count of 76 reads per bin depicted by
            dotted lines. Some regions, e.g. the centromere of chromosome 1 or
            the sub-telomeric regions of chromosomes 13--15, are not covered by
...
\Cref{fig:ss_library} depicts Strand-seq data from a single cell of a \acf{rpe}-1
...
\newcommand\cco{\cellcolor{orange!15}}
    ...
    as Watson fraction; \emph{W.f.}), and the presence of haplotype-tagging
...
includes an example of an inverted duplication within \rpe-1 wild type cells.

\figuretextplusmargin[t!]{mosaic_sv_examples.pdf}{mosaic_examples}{Examples of SVs
    in RPE-1 cells}{Strand-specific count data in 100~kb bins (Watson orange,
...
translocation event. In the \rpe-1 wild
...
bin---specifically, we utilized a \explain{\nb distribution}{
    \marginfig{NB.pdf} Statistical distribution that describes the number of
    successes in a Bernoulli experiment before a given number of failures
    occurs. It is commonly used to describe counts in biological data, e.g.
    by \deseq \citep{Love2014}},
as I describe
...
\figurearbitrary[t!]{0.5}{mosaic_rpe_mean_var.pdf}{mean_var}{Mean variance
    ...
    we estimate $p$ by linear regression without intercept (blue line).}
...
\figuretextplusmargin[t!]{NB_example.pdf}{nb_examples}{Graphical example of the
    ...
    combined, yield a joint probability (area of the rectangles; top right sub-panels).
    ...
    example \textbf{A} would be rejected (WC, i.e. reference, is the most likely
...
performance. Initial results on \rpe-1 cells looked very promising, but did not
...
The simulation consists of three steps. At first, a basic read coverage of each homologue is
...
\figuretextwidth[t!]{mosaic_mc_roc.pdf}{segm_simul}{Performance of segmentation
    on simulated SVs}{Here, I simulated a random number (around 20-50 per
    simulation) of each of three heterozygous \sv classes of
...
I then presented our computational method called \mc that was designed to achieve
...
demonstrating its performance in cell line data. Besides the \rpe-1
