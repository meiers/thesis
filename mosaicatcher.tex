...
\citep{Youssoufian2002}. The relatively high chance of dividing cells to mutate
suggests that nearly all of our cells harbor variation, ranging from \acp{snv}
to large \acp{sv} \citep{Campbell2015}. Fortunately, the majority of these
accumulated variants do not affect our health. Some cases of mosaicism are even
part of our physiological development, such as the programmed rearrangements of
the T-cell receptor genes or polyploidy in liver cells
...
role in that process \citep{Forsberg2017}. The prime example is cancer, which
...
observed Meagabase-range aberrations in people of age above 60, but not in
...
These single-cell methods typically rely on whole-genome amplification such as
MDA or MALBAC to increase the amount of DNA available for sequencing
\citep{Dean2002,Zong2012}. However, the amplification step has been noted to
...
\citep{Garvin2015,Gao2016,Bakker2016,Knouse2016}, copy-neutral \acp{sv} have
been lagging behind.
...
can and will be done using this new method.
...
The simulation consists of three steps. First, the backbone of each homologue is
...
typical sizes of human chromosomes 1--22, X and Y are generated this way.
This is done for a user-defined number of cells.
...
counts per bin: the reads sequenced in dominant the orientation that the
homologue inherits, called \emph{forward} for now, and reads sequenced
...
by a \sv also only partially will have their read counts altered also only
partially. Variants with frequency $f<1$ are incorporated only into subset
of cells, chosen with a probability $f$ for each cell. The user-defined
variant frequency $f$ hence states only the \emph{expected} fraction of cells
...
\begin{table}[t]
    ...
    \tabcap{simulation_params}{Major parameters controlling Strand-seq simulations}
        ...
        $a$ is hence only losely related to $\alpha$ (classification).}
\end{table}
...
within each bin) to be phased, i.e. to virtually overlap a haplotype-phased
\snv. Hence, we know both strand orientation as well as homologue for this
...
each simulation, 200 cells had been generated with an expected number of 20
reads within 50~kb bins. I then distributed one of three \sv classes, namely
...
rather spans one bin completely and two neighboring bins partially. After that,
I applied the segmentation algorithm with increasing numbers of allowed segments
(this is a parameter of the algorithm) to all these data sets. Too few segments
are bound to miss some \sv breakpoints, whereas too many segments only add false
...
\figuretextwidth[t!]{mosaic_mc_roc.pdf}{segm_simul}{Performance of segmentation
    ...
    of allowed segments in the range from 10-50. \Acp{sv} are present in all
    cells.}
...
Smaller \acp{sv} are expectedly more difficult to capture, yet even within the
100~kb events, around three quarter of the breakpoints could be detected while
generating less than 20 false breakpoints. At a size of 400~kb, again all
...
Moreover, as Strand-seq operates on single cells, these \acp{sv} can be detected
...
Previously, these studies had been limited to only \acp{cnv} and subclonal
fractions of at least 10\% due to technical limitations of the applied
approaches. \todo{match with intro to this chapter!}
...
results suggests that the lower detection limit will likely be in the size range
of 100-500~kb.
...
Third, the current segmentation algorithm requires the number of segments as an
...
for our purpose. Moreover, we do not yet know how sensitive the segmentation is
towards low variant frequencies---in case it performs poorly, we will revisit
single-cell segmentation again. Also, the segmentation relies on the bin
coordinates (typically 50~kb), which only sporadically align with \sv
breakpoints. In order to overcome this, we are experimenting with a shifted
...
\sv state having the highest likelihood.
single-cell classifier into rejecting the null hypothesis of a reference state.
To tackle this issue, we want to introduce a size-adjusted prior distribution.
Further, we are considering to restrict the model to bi-allelic \acp{sv}---i.e.
only only variant class at the same time at each locus. This way, single outlier
cells, such as spurious triplication calls in a duplication locus, could be
...
which is tissue-specific---this aspect is further explored by colleagues in the
laboratory. Because of that, a one-fits-all solution (such as the widely-used GC
correction) will likely belie expectations. We are currently testing methods
for normalization based on control samples or on within the data itself using
methods such factor analysis \citep{Stegle2012} or an expectation-maximization
algorithm.
...
published, high-quality set of Strand-seq libraries (Chaisson2017). This data
...
multiple hyperploid cell lines available, including the aforementioned \rpe C29
...
already planned. For example, \ashley recently sequenced cells clones derived
