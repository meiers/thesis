...
Over the past years we have gained more insight into a mechanism in which
...
series of perturbation studies support this hypothesis, as described
subsequently.
...
Moreover, as \cite{Lupianez2016,Krijger2016} review, a number of studies
...
the binding site of an architectural protein in proximity of an enhancer element
and observed looping as well as ectopic gene expression across this boundary.
Similarly, \cite{Narendra2015} observe heterochromatin spreading when they
...
mice using the CRISPR/Cas9 system they observed large changes in chromatin
...
factor. This is why a central foundation of this study was to compare wild
...
the balancer- or wild type chromosomes and thus distinguish up- and
down-regulated genes of the balancer chromosomes in respect to the wild type
expression. \Cref{sec:balancer_maternal_rna,sec:balancer_ase_impl} provide
...
this to observed \ase. To this end we needed to measure
chromatin conformation in a haplotype-resolved manner, too. We hence performed a \hic
...
the context of local mappability, and the \baf.                                 \todo{Exlain \baf in the INTRO}
...
samples, as well as a characteristic change in the \baf. In a
...
I further aimed at refining breakpoint junctions of the major rearrangements on     \todo{Alek: include recent results from Miller et al \url{http://www.g3journal.org/content/ggg/early/2018/02/02/g3.118.200021.full.pdf}}
...
the respective inversions - the last breakpoint appears to lie outside the
mappable reference genome and is thus inaccessible to read-mapping based methods.
...
breakpoint with the genomic positions stated in a recent study mapping mutations        \todo{here new Miller paper}
on \ac{tm3} \citep{Miller2016} I found precise agreement for 8 of 10
breakpoints. We provide a ``best guess'' of the breakpoint on \ac{tm3} at
approximately 20.3~Mb based on \hic information and manual inspection of mate
pair data. Only one of the breakpoint junctions in the balancer
...
breakpoint on \ac{tm3} at 20.3~Mb contains a deletion of around 17~kb, which could
...
extensions of this idea will likely lead towards \explain{reference graphs}{
    represent different alleles in a reference genome simultaneously in order
    to reduce read mapping bias. Such graphs could contain two different alleles,
    but also all haplotypes present in a population. However, this requires new
    algorithms for the interpretation of sequencing data, which the topic is
...
\figuremargin{ASE_chrX_numbers.pdf}{ase_chrX}{ASE fractions per chromosome}{        % move this up or down in the final layout!
    \acs{ase} fraction detected on each of the chromosomes 2, 3, and X in a
    female adult line $F_1^{f}$. Error bars indicate 99\% confidence intervals
...
Next, I determined \ase genes only on chromosome 2 using two replicates of \Fcyo
...
After we characterized \acp{sv} and detected differentially expressed genes we
...
Following the intuition that \ase genes could be located at the junctions of
