...
In the context of \explain{expression quantitative trail loci}{Genomic loci
    with different alleles that influence the expression of genes (typically a
    single gene) that are not necessarily in close proximity} analyses,
...
interactions inside, as \citet{Ji2016} call them, insulated neighborhoods. As
...
conformation, ectopic gene expression across the boundary and indeed the same
...
may lead to severe misregulation via en enhancer adoption mechanisms.
...
\figuretwocolumns[0.38]
    {balancer_chroms.pdf}{balancer_chroms}{Genome overview of balanced fly line}
        {Schematic of the
...
I utilized \wgs data of both $F_0$ and $F_1$ cross simultaneously to call
...
substitutions, i.e. the \explain{mutation spectrum}
    ...
    downstream). The spectrum of somatic \acp{snv} is often analyzed in cancer
...
\begin{table}[th]
    ...
    \begin{tabu}{lrrrr}
        ...
        \bottomrule
    \end{tabu}
    \tabcap{pcrresults}{Deletion validation via PCR}{Number of calls as
        ...
        $^*$ Calls below 160~bp were not \pcr-validated and did not enter the weighted empirical \fdr)}
\end{table}
...
ones. Interestingly, a study in human cancer reported an increase in deletion
...
\figuretextwidth[t]{dup_hic_biaf.png}{dup_hic_biaf}{Large duplication on
    CyO characterized by differential Hi-C}{
    ...
    B-allele frequency. This duplication was initially discovered
    based on the characteristic B-allele frequency signal and is validated by
...
resolution\footnote{\label{footnote:balancer_karyotype}
    Described in terms of cytogenetic bands, i.e. \cyo = \textit{2Lt-22D1
...
the diagonal with a characteristic ``bow tie'' shape that coincided with gaps
...
(blue, i.e. decrease on the balancer chromosome), suggesting that this end of the locus is no longer
...
\figuretextplusmargin{balancer_hiC_rearrangements.png}{balancer_hic_rearrangements}
    {Major rearrangements of balancer chromosomes seen in Hi-C}{
    ...
    characteristic ``bow tie-shaped'' patterns as well as gaps on the diagonal
    that demarcate breakpoint junctions of the rearranged balancer chromosomes.
    The respective sections of the reference genome are color-coded, with a gray
    circle representing centromeres.
    \subpanel{B} These junctions can be followed to reconstruct the relative order of the
    balancer chromosomes such as shown in the top panel. Vertical ``bow ties''
...
over-representation of wild type mRNA. Only after discussions with
...
to \explain{maternally deposited mRNA}{Before maternal-to-zygotic transition
    ...
    is actively degraded, yet some maternal transcript can remain
    \citep{Tadros2009}}
...
extensions of this idea are pointing towards \explain{reference graphs}{
    ...
    could be represented in such a data structure to avoid a reference bias.
...
\begin{table}[t]
    ...
    \begin{tabu}{lrrrr}
        \toprule
        Data set   & Total reads   & Balancer   & WT   & Discordant \\
        \midrule
    ...
    \tabcap{balancer_rnaseq}{Haplotype separation of RNA-seq data}{
        ...
        the haplotypes balancer or wild type (WT) or that contained conflicting
        variants (Discordant). The difference to 100\% in each row comes from
...
also detect if observed alleles within the fragment are discordant---a fact
...
\figuretextwidth{DESeq2_N1_6_8h.png}{DESeq}{ASE analysis based on
    \texorpdfstring{\acs{deseq}}{DESeq2}}{The x-axis shows the average fragment count
...
\figuremargin{ASE_chrX_numbers.pdf}{ase_chrX}{ASE fraction per chromosome}{
...
(Draper-Shark-mediated signaling immune response pathway), \textit{GlyP}
(carbohydrate metabolism pathway) and \textit{p53} (tumor suppressor). The
...
out of 6 neighboring genes, which span a region of around $\pm$~50~kb, were significant.
...
\figuretextwidth[t]{ase_clustering.pdf}{ase_clustering}{Distance between
    ...
    genes (gray), which were sampled from expressed, but non-ASE genes 500 times.
    Error bars indicate the 5\% and 95\% quantiles of random sub-sampling.}
...
Yet it cannot be expected that a higher copy number necessarily leads to an
...
The specific explanation for each gene could, if need to, be unraveled
...
In a thorough manual data inspection, \yad found a sizable number of genes with
...
mRNA is probably just not detectable. At last, it should be kept in mind that
...
\figuretextwidth[t]{HiC_ASE_genes_Alek.pdf}{HiC_ASE_genes}{Differential Hi-C
    contact from gene promoters}{This shows differential \hic contacts between gene
    ...
    genes and plotted here including 95\% confidence intervals.}
...
\figuretextplusmargin{ggbio_dm6_2R_14Mb.jpg}{ggbio1}{Integrated visualization
    ...
    Visualized through our custom plot functions based on
    \ggbio. The figure is explained in the main text.}
...
of the large balancer rearrangements. These plots were highly useful to confirm
...
Based on the study design and experiments of \yad, I deeply
...
active enhancer elements was juxtaposed to potential target genes.
